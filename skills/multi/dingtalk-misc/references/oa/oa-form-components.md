...
| 字段 | 取值规则 |
|------|---------|
| `id` | 套件 `props.id`（服务端优先按 id 精确匹配控件；与 PC 端提交数据的 key 字段同源） |
...
> - 哺乳假模板（options/类型名称含「哺乳」）与需上传证明材料的请假类型（vacation types 返回的类型 item 含 `leaveCertificate.enable` 且换算小时后时长 ≥ 阈值）**不支持 CLI 发起**，引导用户在客户端提交。
...
| 字段 | 取值规则 |
|------|---------|
| `id` | 子控件 `children[].props.id`（服务端按 id 精确匹配；与 PC 端提交数据的 key 字段同源） |
| `name` | 子控件 `props.label`（如「补卡时间」；id 主通道时的回退匹配） |
| `value` | 按**子控件 props.format** 格式化最终补卡时刻毫秒值（默认 `yyyy-MM-dd HH:mm`，禁硬编码格式） |
| `extValue` | 班次数据 JSON 字符串（见下方组装规则；PC 端 extendValue 对象 → MCP 字符串） |
