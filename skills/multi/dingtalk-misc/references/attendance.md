...
| Shortcut | 风险 | 适用场景 |
|---|---|---|
| `dws attendance +calculate-approve-duration` | read | 计算加班/请假/外出等考勤审批时长 |
| `dws attendance +check-companion-schedules` | read | 校验出差/外出同行人的班次是否允许一起提交 |
...
| `dws attendance +get-complex-overtime-setting` | read | 查询员工在指定日期适用的复杂加班规则 |
